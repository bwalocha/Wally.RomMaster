	...
	public class DatListDetailsModel : Microsoft.AspNetCore.Components.ComponentBase
	{
			...
			GridOptions = new GridOptions<GameViewModel>
						{
							...
							Columns = new GridColumn<GameViewModel>[]
									{
										new GridColumn<GameViewModel>
										{
											Caption = "Id",
											Bind = (a) => new MarkupString($"<a href='/games/{a.Id}'>{a.Id}</a>")
										},
										new GridColumn<GameViewModel>
										{
										...
										new GridColumn<GameViewModel>
										{
											...
											Bind = (a) => new MarkupString(
												a.RomCount.ToString(System.Globalization.CultureInfo.InvariantCulture))
										},
										new GridColumn<GameViewModel>
										{
											Caption = "Year", Bind = (a) => new MarkupString(a.Year)
										},
										new GridColumn<GameViewModel>
										{
											Caption = "Description", Bind = (a) => new MarkupString(a.Description)
										},
									}
						};
		}
		...
		protected override async Task OnParametersSetAsync()
		{
			...
			using (var uow = UnitOfWorkFactory.Create())
			{
				...
				var model = await repoDat.FindAsync(
					a => a.Id == Id,
					c => c.Include(m => m.Games).ThenInclude(m => m.Roms)).ConfigureAwait(false);
