...
namespace Wally.RomMaster.DatFileParser.ClrMameProParser
{
	internal class Parser
	{
		...
		public async Task<Models.DataFile> ParseAsync(Stream stream, CancellationToken cancellationToken)
		{
			...
			var response = new Models.DataFile { Header = header };
		...
		private static async Task<Models.Header> ReadHeaderAsync(
		...
		private static async IAsyncEnumerator<Models.Game> ReadGamesAsync(
			IAsyncEnumerator<string> lines,
			CancellationToken cancellationToken)
		{
			while (await lines.MoveNextAsync())
		...
		private static async Task<Models.Game> ReadGameAsync(
			IAsyncEnumerator<string> lines,
			CancellationToken cancellationToken)
		{
			var line = lines.Current;
		...
		private static Task<Models.Rom> ReadRomAsync(
			IAsyncEnumerator<string> lines,
			CancellationToken cancellationToken)
		{
			var line = lines.Current;
			...
			while (key != ")")
			{
				...
				key = tags[0];
				value = tags[1].StartsWith('"')
					? tags[1].Substring(1, tags[1].IndexOf('"', 1)).Trim('"')
					: tags[1].Split(' ', 2)[0];
		...
		private class AsyncLineEnumerator : IAsyncEnumerator<string>
		{
			...
			public async ValueTask<bool> MoveNextAsync()
			{
				...
				this.Current = (await this._reader.ReadLineAsync().ConfigureAwait(false)).Trim();
