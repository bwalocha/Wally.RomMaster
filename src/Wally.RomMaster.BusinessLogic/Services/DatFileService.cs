...
namespace Wally.RomMaster.BusinessLogic.Services
{
	...
	{
		private readonly Parser _datFileParser;

		public DatFileService(
			ILogger<DatFileService> logger,
		...
		{
			this._datFileParser = datFileParser;
		}
		...
		protected override async Task PostProcessAsync(File file, CancellationToken cancellationToken)
		{
			...
			try
			{
				datFile = await this._datFileParser.ParseAsync(file.Path, cancellationToken).ConfigureAwait(false);
			}
			catch (Exception ex)
			{
			...
			{
				var g = new Game { Name = game.Name, Description = game.Description, Year = game.Year };
